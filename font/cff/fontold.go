...
var _ font.Font = (*InstanceOld)(nil)

// InstanceOld represents a CFF font.
type InstanceOld struct {
	*sfnt.Font
	*font.Geometry
...
// Old turns a sfnt.Font into a PDF CFF font.
//
...
func Old(info *sfnt.Font, opt *font.Options) (*InstanceOld, error) {
	if !info.IsCFF() {
...
func (f *InstanceOld) Layout(seq *font.GlyphSeq, ptSize float64, s string) *font.GlyphSeq {
	if seq == nil {
		seq = &font.GlyphSeq{}
	}
...
func (f *InstanceOld) Embed(rm *pdf.ResourceManager) (pdf.Native, font.Embedded, error) {
	opt := f.Opt
