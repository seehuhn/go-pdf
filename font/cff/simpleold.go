...
type embeddedSimpleOld struct {
	embedded

	*encoding.SimpleEncoder
}

func (f *embeddedSimpleOld) DecodeWidth(s pdf.String) (float64, int) {
	if len(s) == 0 {
		return 0, 0
	}
	gid := f.Encoding[s[0]]
...
func (f *embeddedSimpleOld) AppendEncoded(s pdf.String, gid glyph.ID, text string) (pdf.String, float64) {
	width := f.sfnt.GlyphWidthPDF(gid) / 1000
	c := f.SimpleEncoder.GIDToCode(gid, text)
	return append(s, c), math.Round(width*1000) / 1000
}

func (f *embeddedSimpleOld) Finish(rm *pdf.ResourceManager) error {
	if f.closed {
		return nil
	}
	f.closed = true
