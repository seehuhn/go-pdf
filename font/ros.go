...
func ExtractCIDSystemInfo(c pdf.Cursor, obj pdf.Object, _ bool) (*cid.SystemInfo, error) {
	dict, err := c.Dict(obj)
	if err != nil {
		return nil, err
	}
	...
	s, e1 := c.String(dict["Registry"])
	registry = string(s)
	s, e2 := c.String(dict["Ordering"])
	ordering = string(s)

	supplement, e3 := c.Integer(dict["Supplement"])
