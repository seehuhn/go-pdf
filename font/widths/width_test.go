...
func TestWidthsFull(t *testing.T) {
	...
	for _, g := range gg.Seq {
		ww = append(ww, otf.GlyphWidthPDF(g.GID)/1000)
		s, _ = E.(font.EmbeddedLayouter).AppendEncoded(s, g.GID, string(g.Text))
	}
	err = rm.Close()
	if err != nil {
		t.Fatal(err)
	...
	i := 0
	for info := range D.Codes(s) {
		wFromFont := ww[i]
		wFromPDF := info.Width / 1000
		if math.Abs(wFromPDF-wFromFont) > 0.0005 {
			t.Errorf("widths differ for pos %d: %f vs %f", i, wFromPDF, wFromFont)
		}
		i++
	}
}
