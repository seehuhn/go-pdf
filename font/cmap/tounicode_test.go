...
func TestExtractToUnicode(t *testing.T) {
	...
	child, err := pdf.Decode(pdf.NewCursor(data), childRef, ExtractToUnicode)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtractToUnicodeLoop(t *testing.T) {
	...
	for n := 1; n <= 3; n++ {
		t.Run(fmt.Sprintf("%d", n), func(t *testing.T) {
			...
			info, err := pdf.Decode(pdf.NewCursor(data), refs[0], ExtractToUnicode)
			if err != nil {
				t.Fatal(err)
			}
			for i := 0; i < n; i++ {
...
func TestEmbedToUnicode(t *testing.T) {
	...
	info, err := pdf.Decode(pdf.NewCursor(data), ref, ExtractToUnicode)
	if err != nil {
		t.Fatal(err)
	}
