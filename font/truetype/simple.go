...
func (info *PDFFont) Embed(w pdf.Putter, fontDictRef pdf.Reference) error {
	...
	for i := 0; i < 256; i++ {
		encoding[i] = trueTypeFont.GlyphName(info.Encoding[i])
		builtin[i] = trueTypeFont.GlyphName(cffOutlines.Encoding[i])
	}
	...
	fontDescriptor := pdf.Dict{
		...
		"FontName":    fontName,
		"Flags":       pdf.Integer(font.MakeFlags(trueTypeFont, true)),
		"FontBBox":    fontBBox,
	...
	_, err = trueTypeFont.WriteCFFOpenTypePDF(fontFileStream)
	if err != nil {
