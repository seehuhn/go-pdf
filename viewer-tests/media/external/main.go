...
// Command external writes test.pdf, a single landscape page driving a
// movie referenced as an external file (no data embedded in the PDF)
// through a Screen annotation and a Rendition action.  The movie
// carries a video timestamp counter and an audio track that ticks once
// per second.
//
...
// test covers the same rendition operations with the movie embedded in
...
func createDocument(filename string) error {
	...
	page.TextShow("Rendition action test (external movie)")
	page.TextSetFont(bodyFont, 12)
	page.TextSetMatrix(matrix.Translate(80, paper.URy-72))
	page.TextShow("Click the screen to play. The movie is not embedded; the viewer must resolve movie.mp4 next to the PDF.")
	page.TextEnd()
	...
	screen := &annotation.Screen{
		Common: annotation.Common{
			Rect:     screenRect,
			Contents: "Tick movie (320x240, H.264/AAC, 30 s), external file movie.mp4.",
...
// newRendition builds the media rendition that plays the external file
// movie.mp4, with the player's controller UI shown.  The file
// specification has no embedded data; viewers must resolve the name
