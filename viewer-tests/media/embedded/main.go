...
// Command embedded writes test.pdf, a single A5 landscape page with a
// Screen annotation (§13.4.5) over the playback area, driven by rendition
// actions (§12.6.4.10).  The clip is embedded in the PDF file.
//
...
// screen annotation through the action's AN entry.  The page layout is
...
const (
	title = "Rendition action, embedded clip"
	what  = "Click the outlined area to play the movie in place; the buttons drive the rendition operations."
	how   = "The clip is embedded in the PDF file, so a viewer which supports renditions has the data."
)
...
// newRendition builds the rendition which plays the clip embedded in the
// PDF file, with the player's controller UI shown.
func newRendition() *media.MediaRendition {
	return &media.MediaRendition{
		...
		Clip: &media.MediaClipData{
			Name:        "Tick movie",
			DataFile:    moviepage.EmbeddedSpec(),
