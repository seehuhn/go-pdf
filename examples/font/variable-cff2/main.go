...
const (
	fontURL    = "https://cdn.jsdelivr.net/gh/adobe-fonts/source-serif@4.005R/VAR/SourceSerif4Variable-Roman.otf"
	fontFile   = "font.otf"
	outputFile = "test.pdf"
)
...
func run() error {
	...
	for row, opsz := range opticalSizes {
		for col, wght := range weights {
			opt := &embed.Options{
				Composite: true, // there is more than one private cff DICT
