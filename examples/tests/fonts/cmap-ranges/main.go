...
func generateSampleFile(fname string) error {
	...
	cmap := &cmap.InfoNew{
		...
		CodeSpaceRange: []charcode.Range{
			{Low: []byte{0x00}, High: []byte{0x01}},
			{Low: []byte{0x02, 0x00}, High: []byte{0x02, 0xFF}},
			{Low: []byte{0x03}, High: []byte{0xFF}},
		},
	...
	F, err := NewFunnyFont(out.RM, cmap)
	if err != nil {
		return err
	}
	...
	out.TextShowRaw(pdf.String{0x01, 0x03})
	out.TextEnd()
...
func NewFunnyFont(rm *pdf.ResourceManager, cmap *cmap.InfoNew) (*funnyFont, error) {
...
func (f *funnyFont) Embed(rm *pdf.ResourceManager) (pdf.Object, font.Embedded, error) {
...
func (f *funnyFont) CodeAndWidth(s pdf.String, gid glyph.ID, rr []rune) (pdf.String, float64, bool) {
