...
func testVisibilityExpressionRoundTrip(t *testing.T, version pdf.Version, original VisibilityExpression) {
	...
	extracted, err := ExtractVisibilityExpression(pdf.CursorAt(extractor, nil), obj, false)
	if err != nil {
...
func TestVisibilityExpressionCycle(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			go func() {
				_, err := pdf.Decode(pdf.CursorAt(x, nil), veRef, ExtractVisibilityExpression)
				done <- err
			}()
...
func FuzzVisibilityExpression(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, fileData []byte) {
		...
		ve, err := ExtractVisibilityExpression(pdf.CursorAt(extractor, nil), objPDF, false)
		if err != nil {
