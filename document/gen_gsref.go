...
// TestTextShowRaw and TestTextShowRaw2.  For each test case it lays out the
// text, records the glyph x positions, and verifies with ghostscript that the
// different ways of placing the glyphs produce identical images.  The validated
// positions are written to gsref_test.go, so the tests run without ghostscript.
//
...
func run() error {
	...
	return write("gsref_test.go", showRaw2, want)
}
...
func write(fname string, showRaw2 map[string][]float64, showRaw [][]float64) error {
	var b bytes.Buffer
