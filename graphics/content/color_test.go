...
func TestColorOperators_DeviceGray(t *testing.T) {
	...
	// Set stroke gray
	opG := Operator{Name: OpSetStrokeGray, Args: []pdf.Object{pdf.Real(0.5)}}
	...
	// Set fill gray
	opg := Operator{Name: OpSetFillGray, Args: []pdf.Object{pdf.Real(0.75)}}
	if err := state.Apply(res, opg); err != nil {
		t.Fatalf("g operator failed: %v", err)
	}
...
func TestColorOperators_DeviceRGB(t *testing.T) {
	...
	op := Operator{
		Name: "rg",
		Args: []pdf.Object{pdf.Real(1.0), pdf.Real(0.0), pdf.Real(0.0)},
	}
...
func TestColorOperators_SetColorSpace(t *testing.T) {
	...
	opCS := Operator{Name: OpSetStrokeColorSpace, Args: []pdf.Object{pdf.Name("CS1")}}
