...
func extractFontType1(c pdf.Cursor, obj pdf.Object) (*dict.Type1, error) {
	...
	fdDict, err := c.DictTyped(fontDict["FontDescriptor"], "FontDescriptor")
	if pdf.IsReadError(err) {
		return nil, err
	}
	fd, err := pdf.DecodeOptional(c, fdDict, font.ExtractDescriptor)
	if err != nil {
		return nil, err
	}
	...
	d.FontFile, err = glyphdata.ExtractFontFile(c, fdDict, "Type1")
	if err != nil {
		return nil, err
	}
	...
	enc, err := encoding.ExtractSimple(c, fontDict["Encoding"], nonSymbolicExt)
	if err != nil {
		return nil, err
	}
	d.Encoding = enc
...
func repairType1(d *dict.Type1, r pdf.Getter) {
	if d.Descriptor == nil {
