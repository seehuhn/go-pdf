...
func (state *State) Apply(res interface{}, op Operator) error {
...
func (s *State) markIn(bits StateBits) {
	s.In |= (bits &^ s.Out)
}

func (s *State) markOut(bits StateBits) {
	s.Out |= bits
}
...
type opHandler func(*State, []pdf.Native, interface{}) error
