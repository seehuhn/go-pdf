...
// ExtractSigSeedValue reads a seed value dictionary from a PDF file.
func ExtractSigSeedValue(x *pdf.Extractor, path *pdf.CycleCheck, obj pdf.Object, _ bool) (*SigSeedValue, error) {
	...
	if ff, err := pdf.Optional(x.GetInteger(path, dict["Ff"])); err != nil {
		...
		sv.Flags = SigSeedValueFlags(uint32(ff))
	}
	...
	if cert, err := pdf.ExtractorGetOptional(x, path, dict["Cert"], ExtractSigCertSeedValue); err != nil {
		return nil, err
	} else {
		sv.Cert = cert
	}
