...
func drawLineEnding(b *graphics.ContentStreamBuilder, style annotation.LineEndingStyle, info lineEndingInfo) {
...
func (le none) draw(b *graphics.ContentStreamBuilder) {
	if !le.IsStart {
...
func (le butt) draw(b *graphics.ContentStreamBuilder) {
	n := le.Dir.Normal()
	n.IMul(le.size(b.LineWidth) / 2)
...
func (le slash) draw(b *graphics.ContentStreamBuilder) {
	a := 0.5              // cos(60°)
	bb := math.Sqrt(3) / 2 // sin(60°)
	n := vec.Vec2{X: a*le.Dir.X - bb*le.Dir.Y, Y: a*le.Dir.Y + bb*le.Dir.X}
	n.IMul(le.size(b.LineWidth) / 2)
...
func (le square) draw(b *graphics.ContentStreamBuilder) {
	size := le.size(b.LineWidth)
...
func (le circle) draw(b *graphics.ContentStreamBuilder) {
	size := le.size(b.LineWidth)
...
func (le diamond) draw(b *graphics.ContentStreamBuilder) {
	size := le.size(b.LineWidth)
...
func (le arrow) draw(b *graphics.ContentStreamBuilder) {
	tip, base1, base2 := le.corners(b.LineWidth)
	...
	if le.reverse {
		...
		v, _ := linalg.Miter(base1, tip, base2, b.LineWidth, false)
