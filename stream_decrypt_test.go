...
func testEncryptedStreamMultipleReads(t *testing.T, version Version) {
	...
	// Verify that the stream's encryption info is stored (not applied)
	if stream.enc == nil {
		t.Fatal("stream.enc should not be nil for encrypted stream")
	}
...
func TestEncryptedStreamSeekability(t *testing.T) {
	...
	// Verify that Stream.R is seekable
	seeker, ok := stream.R.(io.Seeker)
	if !ok {
		t.Fatal("stream.R should be seekable")
	}

	// Seek to beginning should work
...
func TestEncryptedStreamWithFilters(t *testing.T) {
	// Create a larger test data to ensure compression is effective
...
func TestUnencryptedStreamMultipleReads(t *testing.T) {
	...
	// Verify that unencrypted streams have nil encryption info
	if stream.enc != nil {
		t.Error("stream.enc should be nil for unencrypted stream")
	}
