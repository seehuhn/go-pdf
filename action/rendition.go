...
func decodeRendition(c pdf.Cursor, dict pdf.Dict) (*Rendition, error) {
	an, _ := dict["AN"].(pdf.Reference)

	r, err := pdf.DecodeOptional(c, dict["R"], media.ExtractRendition)
	if err != nil {
		return nil, err
	}
	...
	if dict["OP"] != nil {
		if opVal, err := c.Integer(dict["OP"]); err == nil && opVal >= 0 && opVal <= 4 {
			op.Set(uint(opVal))
		}
