...
func extractType3(c pdf.Cursor, d pdf.Dict) (*Type3, error) {
	...
	bounds, err := getFloatArray(c, d["Bounds"])
	if err != nil {
		return nil, err
	}
	...
	functionsArray, err := c.Array(d["Functions"])
	if err != nil {
		return nil, err
	}
	...
	for i, funcObj := range functionsArray {
		fn, err := pdf.Decode(c, funcObj, Extract)
		if err != nil {
			return nil, err
		}
		functions[i] = fn
